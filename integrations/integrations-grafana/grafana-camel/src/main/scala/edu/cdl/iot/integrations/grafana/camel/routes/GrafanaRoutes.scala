package edu.cdl.iot.integrations.grafana.camel.routes


import edu.cdl.iot.common.schema.Schema
import edu.cdl.iot.integrations.grafana.camel.Constants
import edu.cdl.iot.integrations.grafana.camel.Constants.SCHEMA_HEADER
import edu.cdl.iot.integrations.grafana.core.dto.{GrafanaSensorDataDto, HealthCheckDto}
import edu.cdl.iot.integrations.grafana.core.request.{QueryRequest, SearchRequest, TagRequest}
import edu.cdl.iot.integrations.grafana.core.response.AnnotationResponse
  ...
  override def configure(): Unit = {
    ...
    rest("/query")
      ...
      .process((exchange: Exchange) => {
        ...
        exchange.getIn().setHeader(Constants.QUERY_PARTITION_HEADER, queryPartitions)
      })
      .process((exchange: Exchange) => {
        val request = exchange.getIn.getBody(classOf[QueryRequest])
        val schema = exchange.getIn().getHeader(Constants.SCHEMA_HEADER, classOf[Schema])
        val partitions = exchange.getIn().getHeader(Constants.QUERY_PARTITION_HEADER, classOf[List[String]])
