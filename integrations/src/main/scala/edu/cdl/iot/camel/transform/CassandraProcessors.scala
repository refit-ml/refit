...
class CassandraProcessors(private val dao: CassandraDao,
                          private val config: RefitConfig) {
  ...
  private val getSensorReadings: (GrafanaSensorsDto, String, List[String]) => GrafanaSensorDataDto =
    (sensors: GrafanaSensorsDto, sensorId: String, partitions: List[String]) => {
      val data = dao.getSensorData(getEncryptionHelper,
        sensors.projectGuid,
        sensorId,
        partitions)
  ...
  val sendToCassandra: Processor = new Processor {
    ...
    override def process(exchange: Exchange): Unit = {
      ...
      dao.savePrediction(schema, record, data, predictions, actual)
    }
  }
